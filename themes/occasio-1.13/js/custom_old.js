...
class LinkPreviewGenerator {
    constructor() {
        this.previewLinks = document.querySelectorAll('.preview-link');
        if (this.previewLinks.length > 0) {
            this.init();
        }
    }

    async generatePreview(url, linkElement) {
        // Create loading element
        const loadingDiv = document.createElement('div');
        loadingDiv.className = 'link-preview loading';
        loadingDiv.textContent = 'Loading preview...';
        linkElement.parentNode.insertBefore(loadingDiv, linkElement.nextSibling);
        linkElement.style.display = 'none';

        try {
            const response = await fetch(`/preview.php?url=${encodeURIComponent(url)}`);
            const data = await response.json();
            
            if (data.error) throw new Error(data.error);
            
            this.renderPreview(data, loadingDiv);
        } catch (error) {
            console.error('Preview error:', error);
            this.renderFallback(url, loadingDiv, linkElement);
        }
    }
    ...
    renderPreview(data, container) {
        ...
        const previewHTML = `
            ${data.image ? `<img src="${data.image}" class="preview-image" alt="Preview" onerror="this.style.display='none'">` : ''}
            <div class="preview-content">
                <div class="preview-title">${this.escapeHtml(data.title)}</div>
                <div class="preview-description">${this.escapeHtml(data.description)}</div>
                <div class="preview-url">${this.escapeHtml(data.url)}</div>
            </div>
        `;
    ...
    renderFallback(url, container, originalLink) {
        container.className = 'link-preview fallback';
        container.innerHTML = `
            <div class="preview-content">
                <div class="preview-title">External Link</div>
                <div class="preview-description">Click to visit this website</div>
                <div class="preview-url">${this.escapeHtml(url)}</div>
            </div>
        `;
    ...
    init() {
        this.previewLinks.forEach(link => {
            this.generatePreview(link.href, link);
        });
    }
}

// Initialize link previews when page loads
